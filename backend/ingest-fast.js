/**
 * FAST Document Ingestion - Optimized for Free HuggingFace API
 * Processes files with rate limiting to avoid API blocks
 */
...
console.log('=== FAST RAG Ingestion (Free HuggingFace) ===\n');
...
async function main() {
  try {
    ...
    console.log('Starting ingestion (this will take time with free API)...\n');
    ...
    for (const filePath of allFiles) {
      ...
      try {
        ...
        if (points.length > 0) {
          ...
          if (processed % 5 === 0) {
            console.log(`   → Progress: ${totalVectors} vectors uploaded\n`);
          }
        } else {
