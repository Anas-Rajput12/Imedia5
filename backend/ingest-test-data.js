/**
 * Quick test data ingestion for Frequency Tables
 */
...
async function ingestTestData() {
  try {
    ...
    const points = frequencyTablesContent.map((chunk) => {
      // Create a simple pseudo-vector for testing (in production, use real embeddings)
      const pseudoVector = Array(768).fill(0).map(() => Math.random());
