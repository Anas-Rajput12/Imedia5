/**
 * SMART AI Tutor Page - Structured Teaching Flow
 * 
 * Implements mandatory 7-step teaching loop:
 * 1. Confirm year group + topic (Curriculum Lock)
 ...
 * Personalisation Engine: Trigger-based (2 incorrect → simplify, 3 failed → scaffold)
 */
...
import { BookOpen, Lightbulb, FileText, CheckCircle, XCircle, AlertCircle, RefreshCw } from 'lucide-react'
...
// ==================== TEACHING STEPS (NON-OPTIONAL) ====================

type TeachingStep = 
  | 'topic_selection'      // Step 1: Confirm year + topic
  | 'diagnostic'            // Step 2: Diagnostic micro-check
  | 'teach_chunk'          // Step 3: Teach in small chunk
  | 'guided_example'       // Step 4: Guided example
  | 'student_attempt'      // Step 5: Student attempt
  | 'feedback'             // Step 6: Feedback
  | 'mastery_check'        // Step 7: Mastery check
...
export default function TutorPage() {
  ...
  const [personalisationTrigger, setPersonalisationTrigger] = useState<{
    triggered: boolean
   ...
   * AI may ONLY generate content if:
   * - Valid TopicID selected
   * - Retrieved context exists
   * - Confidence > 0.7 threshold
   * 
   * If not: Ask clarifying question. NEVER invent.
   */
  async function validateCurriculumLock(topic: string): Promise<{
    locked: boolean
    topicExists: boolean
    confidence: number
    message?: string
    ...
    try {
      ...
      const content = await ragService.getTopicContent(subject, topic, selectedYear)
      ...
      if (!topicExists) {
        return {
          ...
          topicExists: false,
          confidence: 0,
          message: `I couldn't find "${topic}" in our curriculum.`,
        }
      }
      ...
      if (!hasContent) {
        return {
          ...
          topicExists: true,
          confidence: 0,
          message: `I found "${topic}" but don't have enough content.`,
        }
      }
      ...
      return {
        ...
        topicExists: false,
        confidence: 0,
        message: 'Error accessing curriculum.',
      }
    }
  }

  // ==================== STEP 1 → STEP 2: START SESSION ====================
  ...
  async function handleTopicSelect(topic: string) {
    ...
    try {
      // STEP 1: Validate curriculum lock
      const lockStatus = await validateCurriculumLock(topic)
      setCurriculumLockStatus(lockStatus)

      if (!lockStatus.locked) {
        setError(lockStatus.message || 'Curriculum validation failed')
        return
      }

      // Curriculum locked - proceed to diagnostic
      ...
      const result = await smartAiTeacherService.startSession(topic, type, selectedYear)
  ...
  function displayTeachingChunk(chunk: TeachingChunk) {
  ...
  function displayWorkedExample(example: WorkedExample) {
    let boardContent = `${type.toUpperCase()} • Year ${selectedYear}\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n\nWORKED EXAMPLE\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.personalisation) {
          setPersonalisationTrigger({
            triggered: true,
            adaptation: result.personalisation.adaptation as any,
            message: result.personalisation.message,
          })
  ...
  async function handleMasterySubmit(answers: Record<string, string>) {
    ...
    setIsLoading(true)
    try {
      const correctCount = Object.entries(answers).filter(([_, answer]) => {
        // Simple check - in production, use backend evaluation
        return answer.trim().length > 0
      }).length

      const masteryPercent = Math.round((correctCount / masteryQuestions.length) * 100)
      const passed = masteryPercent >= 70 // 70% pass threshold

      setMasteryPassed(passed)
      setMasteryAnswers(answers)
  ...
  const speakText = (text: string) => {
    if ('speechSynthesis' in window) {
      ...
      // Select voice based on tutor type
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100 flex flex-col">
      ...
      <div className="flex-1 flex gap-4 p-4 overflow-hidden max-w-[1920px] mx-auto w-full">
        ...
        <div className="w-full lg:w-[42%] min-w-[420px] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          ...
          {/* CHAT INPUT (only in student_attempt step) */}
